...
extern void CreateFile(char *name);
extern void PrintPwd();
extern void TheTest();
extern void StartShell();
extern void ChangeDir(char* name);
...
//    ThreadTest();
	...
	argCount = 1;
		...
		printf("%s, %d",*argv,strcmp(*argv, "-ap"));
	...
	else if (!strcmp(*argv, "-new")){ // append from UNIX to Nachos
		ASSERT(argc > 1);
		CreateFile(*(argv + 1));
		argCount = 2;
	}
	...
	else if (!strcmp(*argv, "-mkdir")){ // append from UNIX to Nachos
		ASSERT(argc > 1);
		Mkdir(*(argv + 1));
		argCount = 2;
	}

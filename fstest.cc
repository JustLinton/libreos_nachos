...
void TheTest(){
    for(int i=0;i<10;i++)
        fileSystem->getShell()->getPwdStack()->push(PwdEntry("test", i));
}

void PrintPwd(){
    printf("name:%d\n", fileSystem->getShell()->getPwdStack()->ind);
}

void ChangeDir(char *path){
    if (!fileSystem->changeDir(path)){
        printf("cd: directory %s not found.",path);
    }
}
...
//	printf("start value: %d,  amountRead %d, ", start, amountRead);
    ...
    // break;
    if(result==0){
        printf("error: no more space. errno: %d\n",result);
        break;
    }
...
//	result = openFile->WriteAt(buffer, amountRead, start);
    ...
    if(result<0){
        printf("NAppend error: no more space. errno: %d\n", result);
        break;
    }
//	printf("result of write: %d\n", result);
	ASSERT(result == amountRead);
